...
class Augment:
	...
	def getImageFileList(self, path):
		...
		#imageFileList = [i for i in imageFileList if i[-3:] not in ['txt','tsv'] ]
		#imageFileList = glob.glob(path+'/*png')
		imageFileList = os.listdir(path)
		imageFileList = list(filter(lambda x:'png' in x,imageFileList))
	...
	def loadImage(self, fileName, path): 
		...
		CLASSES AND LABELS'''
		print(self.dirDict[path] / fileName)
		print(fileName)
...
def main():
	...
	while True:
		...
		if choice == '1':
			augment.checkData(path='train')
			
		if choice == '2':
			augment.rotateAugment(path='/media/patrick/Patrick/modified_idd/train_images')
		...
		if choice == '4':
			augment.moveFiles(oldPath='augImages', newPath='train/')
		
		if choice == '5':
			augment.makeTrainFile(path='train/')
